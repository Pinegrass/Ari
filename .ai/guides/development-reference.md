# Ari Detailed Development Reference

This is the pre-compaction root `AGENTS.md`, retained for progressive disclosure. It contains detailed dependencies, routes, models, environment/deployment notes, and conventions. Some inventories can age; current source and `.ai/20-CURRENT-STATE.md` take precedence.

# Ari — Project Specs & Developer Reference

## Compact continuity startup

Read `.ai/00-INDEX.md`, `.ai/20-CURRENT-STATE.md`, `.ai/60-HANDOFF.md`, `.ai/tasks/ACTIVE.md`, and `.ai/STATE.yaml`; then inspect Git status/log and the delta from `last_verified_commit`. Load deeper material only for the task. Do not default to full audits/archives, repeat completed work, confuse reported with verified, or let active memory grow indefinitely. Update the continuity packet before ending substantial work. Preserve all rules below; inspect nested `backend/` and `aritomo-web/` Git state for cross-surface tasks.
