...
export default function PlanningScreen(){
  ...
  return <ScreenShell scrollable edges={['top']} contentContainerStyle={{padding:20,gap:16}}>
    ...
    {p.loading?<ActivityIndicator/>:isPrivate?<Text style={{color:c.ink}}>{t('private')}</Text>:<>
      {p.result?.estimate?<View><Text style={{color:c.ink}}>{t('planningRemaining')}: {money(p.result.estimate.remaining)}</Text><Text style={{color:c.ink}}>{t('planningDaily')}: {money(p.result.estimate.perDay)}</Text></View>:<Text style={{color:c.ink}}>{t(p.result?.status==='missing'?'planningMissing':'planningStale')}</Text>}
