...
export default function NotificationPreferences() {
  ...
  return <View style={{ padding: 16, backgroundColor: '#fff', borderRadius: 16, marginVertical: 16, gap: 12 }}>
    ...
    {value && <>
      ...
      {(['maxPerWeek', 'quietStart', 'quietEnd'] as const).map(key => <View key={key}><Text>{t(key)}{key === 'maxPerWeek' ? ' (0–21)' : ' (0–23)'}</Text><TextInput editable={!busy} accessibilityLabel={t(key)} keyboardType="number-pad" value={String(value[key])} onChangeText={text => setValue({ ...value, [key]: /^\d{1,2}$/.test(text) ? Number(text) : 0 })} style={{ borderWidth: 1, borderColor: '#aaa', padding: 12, borderRadius: 8 }} /></View>)}
