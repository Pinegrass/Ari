...
export default function TrialCard(){
  ...
  useFocusEffect(useCallback(()=>{let active=true;apiRequest<Trial>('/billing/trial').then(v=>{if(active){setValue(v);setError(false);}}).catch(()=>{if(active)setError(true);});return()=>{active=false;};},[attempt,user?.id]));
