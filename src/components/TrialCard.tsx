...
export default function TrialCard(){
  const {t}=useLanguage(),c=useColors(),{refreshFromSession}=useAuth();
  ...
  useEffect(()=>{let active=true;apiRequest<Trial>('/billing/trial').then(v=>{if(active){setValue(v);setError(false);}}).catch(()=>{if(active)setError(true);});return()=>{active=false;};},[attempt]);
